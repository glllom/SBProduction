...
class DBProductModel(models.Model):
    ...
    bom = models.ManyToManyField(DBComponent, through='legacy.production_old.DBBomComponent', related_name='bom')
    sku = models.CharField(max_length=20, unique=True)
    global_type = models.CharField(max_length=1)
    product_type = models.ForeignKey(DBProductType, on_delete=models.CASCADE)
...
class DBCustomizerComponentAdd(models.Model):
    ...
    product_model = models.ForeignKey('legacy.production_old.DBProductModel', on_delete=models.CASCADE, null=True, blank=True)
